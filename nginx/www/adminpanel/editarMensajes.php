<?php
session_start();
// Pagina para editar los Mensajes desde el apartado de administrador 
// Consulta de los atributos del mensaje donde el adminitrador respondió  y actualización como los nuevos cambios 
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = "SELECT * FROM mensajes WHERE idMensaje = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE mensajes SET mensaje = ? WHERE idMensaje = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('si', $mensajeTexto, $id);
    $stmt->execute();
...
<div class="sticky-top">
    ...
    <div class="centrar">
        <div class="titulo">Editar Mensaje</div>
            <form method="POST" class="formulario">
                <input type="hidden" name="id" value="<?php echo $mensaje['idMensaje']; ?>">
                <label class="mt-2">Mensaje:</label>
                <textarea name="mensaje" class="mt-2"><?php echo htmlspecialchars($mensaje['mensaje']); ?></textarea>
                <button type="submit" class="mt-2 boton">Actualizar</button>
            </form>
